...
**Post-v0.1.1 milestone lock (superseded 2026-04-26)**: v0.2 shipped Squad puppets + dev validation tool — see `mvp-phase-c-squad-validation-design.md` and `mvp-phase-c-implementation-plan.md`. Note this is **MVP scaffolding**, not the long-term direction. The post-MVP roadmap is now governed by `06-full-gameplay-vision.md`; real deputy execution will be LLM-driven (vision §2.2, future doc 08), and economy/production work will follow the doc 09 schema (vision §7). Player-side multi-select / drag-box remains explicitly out of scope (vision §6); the v0.2 dev controller is debug-build-only.
