...
- [ ] **Step 1: Audit every `Control` in the HUD and confirm `mouse_filter = MOUSE_FILTER_STOP` on interactive widgets and `MOUSE_FILTER_IGNORE` on decorative containers**
- [ ] **Step 2: In `hero_controller.gd`, gate raycast input on `get_viewport().gui_get_hovered_control() == null`**
- [ ] **Step 3: Smoke test** — click the command panel, voice button, and victory overlay; confirm no hero movement or target acquisition leaks through
- [ ] **Step 4: Commit**
...
- [ ] **Step 1: Add a `Label3D` or `ProgressBar3D` child showing HP, driven by the `hp_changed` signal**
- [ ] **Step 2: On `destroyed`, play a brief tween (scale-down / fade) before `queue_free()` so the destruction reads clearly**
- [ ] **Step 3: Smoke test** — destroy each building in `world.tscn`; confirm HP readout decreases monotonically and destruction is visible
- [ ] **Step 4: Commit**
...
- [ ] **Step 1: Ensure each of the following prints exactly once at the right place:**
...
- [ ] **Step 2: Run the full manual acceptance flow from the smoke-test checklist and confirm the log stream reads cleanly**
- [ ] **Step 3: Commit**
...
- [ ] Run headless boot: `godot4 --headless --path godot --quit` → no errors
- [ ] Run headless tests (once Task 2 lands) → all green
- [ ] Run manual acceptance per [`05-godot-smoke-test-checklist.md`](05-godot-smoke-test-checklist.md) → all items pass
- [ ] Verify acceptance criteria 1-8 from [`01-rts-mvp-design.md`](01-rts-mvp-design.md) §10
